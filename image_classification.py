...
def main():
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],3) * 100  #アザラシの確率(%)
        pred1 = round(prediction[1],3) * 100  # 猫の確率(%)
        pred2 = round(prediction[2],3) * 100  # 犬の確率(%)
        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「アザラシ」です！")
        elif classNo ==1:
            st.subheader(f"これは{pred1}％の確率で「猫」です！")
        else:
            st.subheader(f"これは{pred2}％の確率で「犬」です！")
...
# 学科：高度情報工学科
# 学年：４年
# コメント：そこまで多くないサンプルだけで、これだけの精度を出せるのはすごいと思った
#
#=============================================================================#
